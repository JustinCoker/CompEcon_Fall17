...
def sim_moments(theta):
    ...
    k_sim, I_sim, V_sim, prof_sim, extfin_sim = simul.main(theta, params, kgrid, pi,
                                             Vnext, loc, T, N, z_grid)

    def moments(k_sim, I_sim, V_sim, prof_sim, extfin_sim):
        ...
        ext = extfin_sim.reshape((N*50, 1)).sum() / N*50

        U_sim = np.array((a1[0, 0], a2[0, 0], sc_ik, sd_piK, qbar, ext))

        return U_sim


    U_sim = moments(k_sim, I_sim, V_sim, prof_sim, extfin_sim)
...
theta = np.array([ak, psi, rho, sigma, theta0])

def dist(theta):
    U_sim = sim_moments(theta)
    U_data = np.array((.03, .24, .4, .25, 3.0, .25))

    dist = np.matrix((U_data - U_sim)) * np.eye(6) * np.matrix((U_data - U_sim)).T
